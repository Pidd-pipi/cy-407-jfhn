...
- 展览管理：创建、编辑、删除展览，调整展品顺序，设置主题色并发布。
  - 墙面布置方案：每件已选展品占用 1-12 号唯一墙面槽位，可单独设置朝向与缩放；后墙 1-6、左墙 7-9、右墙 10-12。
  - 发布校验：未覆盖全部展品、槽位重复或超过十二个时发布失败，草稿与既有快照均保留。
  - 发布冻结：成功发布时冻结展品顺序、主题色和布置快照；此后编辑只改草稿，重新发布才同步展厅。
  - 展厅读取最近发布快照；旧展览无布置数据时沿用按顺序的自动排位，刷新后保持一致。
